...
const RfIcon = ({ icon, size = 16, className = '', ...props }) => {
  const selectedIcon = iconMap[icon];
  if (!selectedIcon) return null;

  return (
    <span className={`rf-icon ${className}`} {...props}>
      {React.cloneElement(selectedIcon, { width: size, height: size })}
    </span>
  );
};

// Web Component tanımlaması
if (window?.customElements && !customElements.get('rf-icon')) {
  class RfIconElement extends HTMLElement {
    ...
    render() {
      ...
      const svg = this.getIconSvg(icon);
      if (svg) {
        this.innerHTML = svg.replace(/width="16"/, `width="${size}"`).replace(/height="16"/, `height="${size}"`);
      }
    }
  }

  customElements.define('rf-icon', RfIconElement);
}
